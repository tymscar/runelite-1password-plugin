...
class CommandRunner extends Thread {
	...
	CommandRunner(String accountId, Consumer<String> consumer, boolean isOtpRequest) {
		super(() -> {
			try {
				ProcessBuilder pb = isOtpRequest ? buildOtpCommand(accountId) : buildCommand(accountId);
				pb.redirectErrorStream(true);
				Process p = pb.start();
	...
	private static ProcessBuilder buildOtpCommand(String accountId) {
		List<String> params = new ArrayList<>();
		...
		params.add("op item get " + query + " --otp" + redirect);
